...
const int lightmode = 0;           //1 = day/night   0 = night light mode
...
//LED Variables
int green = 128, green_r = green; //RGB LED settings
int blue = 0, blue_r = blue;      //RGB LED settings
int red = 255, red_r = red;       //RGB LED settings

//LED Pins - Does nto use addressable LEDs, uses 4 wire LED strip, GND RED BLUE GREEN
int red_pin = 0;   //D5
int green_pin = 14 ;  //D4
int blue_pin = 2;  //D3
...
void setup()
{
  ...
  pinMode(green_pin , OUTPUT);  //Blue
  pinMode(blue_pin , OUTPUT);  //Green
...
void DoTheLEDs()
{
  ...
  // clock_minutes = 1400;  // For testing purposes
...
void daynight()
{
  ...
  if (SR_Phase == 1 && blue >= 0)
  {
    ...
    blue = blue_r - (LED_phase * change * 2); //work out 0 - 255 for sunset.  4 Because in 2 phases (/2) and green starts at 128 (/4)
  ...
  if (SS_Phase == 1 && green >= 0)
  {
    ...
    green = green_r - (int)LED_phase; //work out 0 - 255 for sunset.  4 Because in 2 phases (/2) and green starts at 128 (/4)
